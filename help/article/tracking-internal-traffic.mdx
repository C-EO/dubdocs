...
You can now see the [full referer URL](/changelog/referer-urls) data in [Dub Analytics](/help/article/dub-analytics). This lets you understand the specific URL where your users are coming from – as long as the site has the correct [Referer-Policy](#how-to-enable-full-referer-urls) header set.
...
<img src="https://assets.dub.co/changelog/referer-urls.png" alt="Full Referer URLs in Dub Analytics" className="rounded-lg border border-gray-100" />
...
<img src="https://assets.dub.co/help/referer-urls-leads.png" alt="Full Referer URLs in Dub Analytics" className="rounded-lg border border-gray-100" />
...
If you are using [Next.js](/solutions/typescript), you can use the [`headers` property](https://nextjs.org/docs/app/api-reference/next-config-js/headers) in your `next.config.js` file to configure the referer policy.
...
If you are using [Laravel](/solutions/php), you can use the [Laravel Security Middleware](https://laravel-news.com/laravel-security-middleware) to configure the referer policy.
...
If you are using [Flask](/solutions/python), you can set the `Referrer-Policy` header for all requests like this:
