...
## Where to find your program commissions? When viewing your Partner Program
navigation, click on the **Commissions** page, under the **Insights** group:
...
/>
| Column       | Data                  | | ---------- | ---------------------- |
| Date       | Commission date | | Customer        | The customer associated
with the lead or sale   | | Partner   | Your partner | | Type   | The commission
type (custom, click, lead, or sale) | | Amount   | The sale amount, or single
count for custom, click, or lead rewards | | Commission | The amount the partner
will receive | | Status | Current status of the commission. [Learn more about
statuses](https://dub.co/help/article/partner-commissions#commission-statuses).
| | Additional actions | Actions to mark the commission as duplicate, fraud, or
canceled |
...
<img
  ...
  className="rounded-lg border border-gray-100"
/>
### | Input | Description | Required | | --- | --- | --- | | Referral link | The
referral link that the sale is attributed to | ✅ | | Customer | The customer
associated with the sale. [Read more about adding a customer
](/help/article/partner-commissions#creating-a-new-customer) | ✅ | | Event
source | Create a new sale event, or import from Stripe | ✅ | | Sale date | The
date of the sale. If no date is selected, it will auto-fill with the current
date and time | ❌ | | Sale amount | The amount of the sale in USD. | Required
if not [reusing existing events](#reusing-existing-conversion-events) | |
Invoice ID | The invoice ID of the sale. Can be used to [fetch the commission
later via the
API](https://dub.co/docs/api-reference/endpoint/retrieve-a-list-of-commissions#parameter-invoice-id)
| ❌ | | Product ID | The product ID of the sale. Can be used to [fetch the
commission later via the
API](https://dub.co/docs/api-reference/endpoint/retrieve-a-list-of-commissions#parameter-invoice-id)
| ❌ |
