...
Then, you'll need generate a [publishable key](/docs/api-reference/publishable-keys) from your Dub workspace to track conversions on the client-side.
...
Alternatively, you can [track conversion events server-side](/docs/conversions/quickstart#step-3%3A-install-the-dub-server-side-sdk-%2B-track-conversion-events) for [lead events](/docs/conversions/leads/introduction) and [sale events](/docs/conversions/sales/introduction) by sending the `clickId` resolved from the deep link to your backend and then calling off to either:
