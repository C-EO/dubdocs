...
- Building gamified referral programs with [Dub Conversions](/docs/conversions/quickstart) – e.g. increment usage credits for the referrer when a [new signup](/docs/conversions/leads) happens
...
3. **Signing secret**: This is an auto-generated secret key that you can use to verify the authenticity of the webhook in your application. Learn more about [verifying webhook requests](/docs/concepts/webhooks/verify-webhook-requests).
4. **Events**: Select the events you want to listen to. You can select multiple events. Refer to the [Event Types](/docs/concepts/webhooks/event-types) section to see the list of available events.
