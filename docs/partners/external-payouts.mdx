...
This is the default payout mode for new programs. All partners are paid directly by Dub through Stripe Connect & PayPal based on the partner's country. This is the simplest option and requires no additional setup beyond [connecting your bank account](/help/article/how-to-set-up-bank-account).
...
<Frame>
  <img
    src="/images/program-payout-settings-external.png"
...
- [Creating a partner via API](/docs/api-reference/partners/create)
- [Updating a partner in your dashboard](/help/article/managing-program-partners#advanced-settings)
- Importing partners from another platform

<Frame>
  <img
    src="/images/partner-advanced-settings-modal.png"
    alt="Partner tenant ID"
  />
</Frame>
...
Create a webhook endpoint that listens for the `payout.confirmed` event. See our [webhook setup guide](/docs/concepts/webhooks/introduction) for detailed instructions.

Your webhook endpoint should:

1. Verify the [webhook signature](/docs/concepts/webhooks/verify-webhook-requests) to ensure it's from Dub
...
<Accordion title="Partner not receiving external payouts">
  - Verify the partner has a `tenantId` configured - Check that your program's
  payout mode is set to Hybrid or External - Ensure the partner meets the
  minimum payout threshold - Confirm the partner's status is "approved"
</Accordion>
...
- [Webhook setup guide](/docs/concepts/webhooks/introduction)
- [Webhook event types](/docs/concepts/webhooks/event-types)
- [Verifying webhook requests](/docs/concepts/webhooks/verify-webhook-requests)
- [Partners API reference](/docs/api-reference/partners/create)
