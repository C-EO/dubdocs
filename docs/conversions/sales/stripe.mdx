...
Finally, when the user completes the checkout flow, Dub will automatically [track the sale event](/docs/api-reference/analytics/track-sale) and [update the customer's `customerExternalId`](/docs/api-reference/customers/update) with their Stripe customer ID for future reference.
