...
[Webhooks](/docs/webhooks/introduction) are _push-based_, meaning that Dub will send events to your webhook receiver endpoint when specific events occur, while a REST API is _pull-based_, meaning that you need to consistently poll the API endpoint to get real-time updates.
