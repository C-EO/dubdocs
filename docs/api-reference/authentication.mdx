...
Dub uses two types of authentication keys depending on where your code runs. Choose the right one for your use case:

| Key type                                  | Use case                                | Where to use                                |
| :---------------------------------------- | :-------------------------------------- | :------------------------------------------ |
| [**API keys**](#api-keys)                 | Server-to-server (REST APIs)            | Back-end only; store securely, never expose |
| [**Publishable keys**](#publishable-keys) | Client-side (SDKs, conversion tracking) | Safe to include in frontend code            |
