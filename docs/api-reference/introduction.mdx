...
Authentication to Dub's API is performed via the Authorization header with a Bearer token. To authenticate, you need to include the Authorization header with the word `Bearer` followed by your [API key](/docs/api-reference/tokens) in your requests like so:
...
- [retrieve a list of events](/docs/api-reference/analytics/list-events)
